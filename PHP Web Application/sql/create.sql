...
BEGIN
	IF (SELECT count(*) FROM Product where Product.name = `name` and Product.ManufacturerID = `ManufacturerID`) = 0
        ...
        INSERT INTO `Product` (name, price, stock, ManufacturerID, image)
		VALUES (`name`, `price`, `stock`, `ManufacturerID`, `image`);
...
CREATE  INDEX city ON manufacturer(city);
CREATE  INDEX firstName ON customer(firstName);
CREATE  INDEX lastName ON customer(lastName);
CREATE  INDEX name ON manufacturer(name);
