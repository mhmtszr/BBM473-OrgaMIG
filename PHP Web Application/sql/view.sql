...
SELECT p.ID AS ProductID, p.name AS productName, p.price, p.stock, m.ID ,m.name, m.city, p.isDeleted as 'isProductDeleted', p.image , m.isDeleted as 'isManufacturerDeleted'
...
where c.ID = s.CustomerID and s.OrderID = o.ID and p.ID = s.ProductID Order By o.orderDate desc
